bridgecoin Core version 0.13.2 is now available from:

  <https://download.bridgecoin.org/bridgecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/bridgecoin-project/bridgecoin/issues>
...
updates will be released anymore. Without security updates, using a bridgecoin
...
ECDSA signatures inside bridgecoin transactions now use validation using
...
Previous versions of bridgecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of bridgecoin Core was to raise the default minimum
relay fee.

bridgecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
bridgecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bridgecoind without having to do any manual configuration.
...
bridgecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. bridgecoin Core will relay transactions with
...
entirely in bridgecoin Core 0.13, and it is at this time undecided whether the
...
bridgecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
bridgecoin Core automatically creates a hidden service to listen on, without
manual configuration. bridgecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if bridgecoin Core is listening, and
...
bridgecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals bridgecoin
Core to use floating fees. By default, bridgecoin Core will use floating
fees.
...
(default: `0.0002` BCO/kB).

At all times, bridgecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BCO.
Furthermore, bridgecoin Core will never create transactions paying less than
...
`bridgecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bridgecoinrpc>
        ...
        # optional bypass bridgecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bridgecoinrpc:<password>
    </Location>
...
Restarting bridgecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bridgecoin.conf
...
bridgecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bridgecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The bridgecoin Core code base has
...
- `bridgecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bridgecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the bridgecoin scripting system that in combination with
...
bridgecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of bridgecoin Core.
...
The two phases are distinct in the bridgecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming bridgecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for bridgecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each bridgecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bridgecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bridgecoins being spent is exactly
...
- **Increased security for multisig:** bridgecoin addresses (both P2PKH addresses
  ...
  believed to be provided by bridgecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of bridgecoin's current ledger (called the UTXO set),
  ...
  note that bridgecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  bridgecoin users to individually opt-in to almost any change in the bridgecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and bridgecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the bridgecoin
client to bridgecoin Core 0.13.2 to signal for segwit support. When a
...
Since bridgecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `bridgecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bridgecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
