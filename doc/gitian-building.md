...
*Setup instructions for a Gitian build of bridgecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the bridgecoin
...
- [Building bridgecoin Core](#building-bridgecoin-core)
...
Clone the git repositories for bridgecoin and Gitian.
...
in the bridgecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building bridgecoin Core
----------------

To build bridgecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bridgecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bridgecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bridgecoin-project/bridgecoin
...
For example:
```bash
URL=https://github.com/thrasher-/bridgecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bridgecoin=${COMMIT} --url bridgecoin=${URL} ../bridgecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bridgecoin=${COMMIT} --url bridgecoin=${URL} ../bridgecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bridgecoin=${COMMIT} --url bridgecoin=${URL} ../bridgecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bridgecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bridgecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bridgecoin
SIGPATH=/some/root/path/bridgecoin-detached-sigs

./bin/gbuild --url bridgecoin=${BTCPATH},signature=${SIGPATH} ../bridgecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.BCO` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bridgecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bridgecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bridgecoin-osx-build.assert
```
...
[gitian.sigs.BCO](https://github.com/bridgecoin-project/gitian.sigs.BCO/) repository, or if that's not possible create a pull
