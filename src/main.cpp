...
#if defined(NDEBUG)
# error "bridgecoin cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BCO additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("bridgecoin-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // bridgecoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // bridgecoin: (mainnet >= 710000, regtest and testnet uses supermajority)
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // bridgecoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
